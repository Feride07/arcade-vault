...
└── GameWrapper           — обёртка для игр
    ...
    ├── ReactionGame      — React-игра "Рефлекс"
...
В компоненте `ReactionGame` использование `useState` для score/round внутри `setTimeout`-коллбэков приводило к stale closure — функция видела устаревшие значения переменных.
...
8 раундов. В каждом раунде через случайную задержку (1.2–3.4 сек) появляется цель. Игрок должен кликнуть по ней как можно быстрее.

```
Формула очков: pts = max(10, round(1000 - reaction_ms × 0.8))
  100ms  → ~920 очков
  300ms  → ~760 очков
  600ms  → ~520 очков
  1000ms →  200 очков
  >1250ms →  10 очков (минимум)

Штраф за ранний клик: -100 очков
```
...
| `--yellow` | `#ffe500` | Цвет Reflex, достижения |
...
| `speedster` | Молния | реакция < 200ms в Рефлексе |
...
└── GameWrapper           — Game container + overlay
    ...
    ├── ReactionGame      — React component game
...
### 3.3 Stale Closure Solution (ReactionGame)
...
### 4.4 Reflex
- **Rounds:** 8, random delay 1.2–3.4s before target appears
- **Scoring:** `pts = max(10, round(1000 - reaction_ms × 0.8))`
- **Penalty:** `-100` for early click
- **Architecture:** all counters in `useRef` to avoid stale closures
